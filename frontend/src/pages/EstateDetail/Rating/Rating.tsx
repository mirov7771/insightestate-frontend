...
export const Rating: FC<Grade> = ({
  ...
  return (
    <div>
        <h5 className={styles.info_icon}>Наша оценка объекта
      ...
      <div className={styles.rating}>
        {main && (
          <div className={styles.rating__item}>
            ...
            <span className={`${styles.rating__score} ${styles.rating__score_result}`} onClick={openRatingInfo}>
        ...
        {investmentSecurity && (
          <div className={styles.rating__item}>
            ...
            <span className={styles.rating__score} onClick={handleSecurity}>{investmentSecurity.toPrecision(2)}</span>
          </div>
        )}
        {investmentPotential && (
          <div className={styles.rating__item}>
            <span>Инвестиционный потенциал</span>
            <span className={styles.rating__score} onClick={handleInvest}>{investmentPotential.toPrecision(2)}</span>
          </div>
        )}
        {projectLocation && (
          <div className={styles.rating__item}>
            <span>Расположение проекта</span>
            <span className={styles.rating__score} onClick={handleLocation}>{projectLocation.toPrecision(2)}</span>
          </div>
        )}
        {comfortOfLife && (
          <div className={styles.rating__item}>
            <span>Комфорт жизни</span>
            <span className={styles.rating__score} onClick={handleComfort}>{comfortOfLife.toPrecision(2)}</span>
          </div>
        )}
      </div>
        <InfoModal
